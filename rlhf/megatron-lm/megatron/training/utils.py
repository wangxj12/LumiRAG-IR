...
def get_ltor_masks_and_position_ids_yuanvl_train(data,
    ...
    assert eod_mask_loss, 'yuanvl need eod_mask_loss is True'
    # Attention mask (lower triangular).
    if reset_attention_mask:
        att_mask_batch = micro_batch_size
    else:
    ...
    for b in range(micro_batch_size):
        if num_tile[b][0] == 0:
            ...
            eod_index = position_ids[b, data[b] == eod_token]
            num_tile_pre_mbs = num_tile[b]
            assert len(bos_index) == len(eod_index)
            num_sequence = len(bos_index)

            assert num_sequence == len(image_per_sample[b])
            assert image_per_sample.sum() == len(num_tile_pre_mbs)

            image_pos = 0

            image_end_token_index_per_sequence = []
            for j in range(num_sequence):

                image_in_this_sequence = int(image_per_sample[b][j])
                input_pad = input_pad + [bos_token]
                for k in range(image_in_this_sequence):
                    tile_in_this_image = num_tile_pre_mbs[image_pos + k]
  
                    input_pad = input_pad + [image_start_token] + [pad_token] * int(clip_visual_size * downsample_ratio**2) * tile_in_this_image + [image_end_token]
                image_end_token_index_per_sequence.append(len(input_pad)-1)
                image_pos = image_pos + image_in_this_sequence
                start_idx = bos_index[j]
                end_idx = eod_index[j]
                input_pad = input_pad + data[b][start_idx + 2 : end_idx + 1].tolist()

            input_pad = input_pad + [pad_token]

    input_pad_tensor = torch.Tensor(input_pad).view(1, -1).long().cuda()
    data_pad_tensor = input_pad_tensor[:, :-1].contiguous()
    label_pad_tensor = input_pad_tensor[:, 1:].contiguous()
    ...
    if num_tile[b][0] != 0:
        ...
        # Loss mask.
        image_info_pad = {}
        loss_mask = torch.ones(data_pad_tensor.size(), dtype=torch.float, device=data_pad_tensor.device)
        ...
        for b in range(micro_batch_size):
            ...
            assert len(bos_index) == len(sep_index)

        if eod_mask_loss:
            loss_mask[data_pad_tensor == bos_token] = 0.0
            loss_mask[data_pad_tensor == image_start_token] = 0.0

            loss_mask[data_pad_tensor == eod_token] = 0.0
            loss_mask[data_pad_tensor == pad_token] = 0.0
            for b in range(micro_batch_size):
        ...
        if reset_position_ids or reset_attention_mask:
            ...
            for b in range(micro_batch_size):
                ...
                assert len(bos_index) == len(eod_index) == len(image_end_token_index_per_sequence)
                # Loop through EOD indecies:
                prev_index = 0


                for j in range(eod_index.size()[0]):
                    start_idx = image_end_token_index_per_sequence[j]
                    end_idx = eod_index[j]
        ...
        for b in range(micro_batch_size):
            eod_indexs = position_ids[b, label_pad_tensor[b] == eod_token]
            sep_indexs = position_ids[b, label_pad_tensor[b] == sep_token]
            if len(eod_indexs) == len(sep_indexs):
                for ii, eod_index in enumerate(eod_indexs):
                    if ii == 0:
                        sep_index = torch.nonzero(label_pad_tensor[b, :eod_index] == sep_token)
                        if len(sep_index) != 0:
                            sep_index = sep_index[0]
                            loss_mask[b, :sep_index+1] = 0.0
                    elif ii == len(eod_indexs) - 1:
                        sep_index1 = torch.nonzero(label_pad_tensor[b, eod_indexs[ii - 1]:eod_index] == sep_token)
                        sep_index2 = torch.nonzero(label_pad_tensor[b, eod_index:] == sep_token)
                        if len(sep_index2) != 0:
                            sep_index2 = sep_index2[0]
                            loss_mask[b, eod_index + 1:eod_index + sep_index2+1] = 0.0
                        if len(sep_index1) != 0:
                            sep_index1 = sep_index1[0]
                            loss_mask[b, eod_indexs[ii - 1] + 1: eod_indexs[ii - 1] + sep_index1+1] = 0.0
                    else:
                        sep_index = torch.nonzero(label_pad_tensor[b, eod_indexs[ii - 1]:eod_index] == sep_token)
                        if len(sep_index) != 0:
                            sep_index = sep_index[0]
                            loss_mask[b, eod_indexs[ii - 1] + 1:eod_indexs[ii - 1] + sep_index+1] = 0.0
            elif len(eod_indexs) == 0:

                if len(sep_indexs) != 1:
                    print('SEPINDEX:', label_pad_tensor)
                else:
                    assert len(sep_indexs) == 1, '单轮对话只能有一个sep'
                    last_sep_index = sep_indexs[-1]
                    loss_mask[b, :last_sep_index+1] = 0

            else:
                assert len(eod_indexs) < len(sep_indexs)
                sep_indexs_use = []

                for ieod in eod_indexs:
                    idx = bisect.bisect_left(sep_indexs.tolist(), ieod)
                    closest_sep = sep_indexs[idx - 1]
                    sep_indexs_use.append(closest_sep)
                assert len(sep_indexs_use) == len(eod_indexs)

                for ii, eod_index in enumerate(eod_indexs):
                    assert sep_indexs_use[ii] < eod_indexs[ii]

                    if ii == 0:
                        end_idx = sep_indexs_use[0]
                        loss_mask[b, :end_idx+1] = 0.0

                    else:
                        start_idx = eod_indexs[ii - 1]
                        end_idx = sep_indexs_use[ii]
                        loss_mask[b, start_idx+1:end_idx+1] = 0.0
    ...
    return attention_mask, loss_mask, position_ids_use, data_pad_tensor, label_pad_tensor, image_info_pad
