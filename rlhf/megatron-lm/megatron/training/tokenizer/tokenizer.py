...
def build_tokenizer(args, **kwargs):
    ...
    if args.tokenizer_type == 'BertWordPieceLowerCase':
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_model_path, add_eos_token=False, add_bos_token=False, eos_token='<eod>')
        if args.use_yuanvl:
            ...
            special_tokens_list.extend(['<|begin_of_sentence|>', '<|end_of_sentence|>', '<|User|>', '<|Assistant|>', '<think>', '</think>', '<search_result>', '</search_result>', '<search_query>', '</search_query>', '<code_query>', '</code_query>', '<code_result>', '</code_result>', '<infer>', '</infer>', '<inferresult>', '</inferresult>'])
            
        else:
            special_tokens_list = ['<sep>', '<pad>','<mask>','<predict>','<FIM_SUFFIX>','<FIM_PREFIX>','<FIM_MIDDLE>','<commit_before>','<commit_msg>','<commit_after>','<jupyter_start>','<jupyter_text>','<jupyter_code>','<jupyter_output>','<empty_output>', '<repo_name>', '<file_sep>', '<eog>', '</eog>']
