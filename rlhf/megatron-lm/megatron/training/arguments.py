...
def add_megatron_arguments(parser: argparse.ArgumentParser):
    ...
    parser = _add_yuanvl_args(parser)
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    kw_args['use_lf_gate'] = args.use_lf_gate
    kw_args['lf_conv2d_group'] = args.lf_conv2d_group
    kw_args['lf_conv2d_num_pad'] = args.lf_conv2d_num_pad
    kw_args['lf_conv2d_add_bias'] = args.lf_conv2d_add_bias
    kw_args['use_apex_rmsnorm'] = args.use_apex_rmsnorm
    # Return config.
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--lf-conv2d-add-bias', action='store_true',
            help='If set, lf conv2d will add bias.')
    group.add_argument('--use-apex-rmsnorm', action='store_true',
            help='whether to use apex rmsnorm.')
...
def _add_yuanvl_args(parser):
    group = parser.add_argument_group(title="yuanvl")

    group.add_argument('--use-yuanvl', action='store_true',
                       help='whether to use yuanvl')
    group.add_argument('--yuanvl-frozen-llm', action='store_true',
                       help='whether to frozen llm')
    group.add_argument('--yuanvl-use-te-imagemlp', action='store_true',
                       help='whether to use te imagemlp, 40B nees set True')
    group.add_argument('--imagemlp-recompute', action='store_true',
                       help='whether to use recompute in imagemlp')
    group.add_argument('--yuanvl-full-npy-path', type=str, default=None,
                       help='yuanvl dataset npy path')
    group.add_argument('--clip-download-path', type=str, default=None,
                       help='clip download path')
    group.add_argument('--clip-model-name', type=str, default='InternViT-448', choices=['InternViT-448'],
                       help='clip model name')

    group.add_argument('--clip-visual-size', type=int, default=1024,
                       help='clip visual size')
    group.add_argument('--clip-hidden-size', type=int, default=1024,
                       help='clip hidden size')
    group.add_argument('--yuanvl-load-yuanlm-first', action='store_true',
                       help='whether to load language model only')
    group.add_argument('--max-split-tile-num-single-image', type=int, default=9,
                       help='the max number of image tile or num_image = 1 in a sequence')
    group.add_argument('--max-split-tile-num-multi-image', type=int, default=4,
                       help='the max number of image tile for num_image >= 2 in a sequence')
    group.add_argument('--image-segment-method', type=str, default='adaptive',
                       help='the max number of image tile')
    group.add_argument('--downsample-ratio', type=float, default=0.5,
                       help='downsample ratio')
    group.add_argument('--shape-change-threshold', type=float, default=1.0,
                       help='image shape change threshold')
    group.add_argument('--dataset', type=str, default='',
                       help='')
    group.add_argument('--result-path', type=str, default='',
                       help='')
    group.add_argument('--case-name', type=str, default='',
                       help='')
    group.add_argument('--use-loss-chunk', action='store_true',
                       help='whether to use loss chunk')
    group.add_argument('--loss-chunk', type=int, default=1024,
                       help='if use loss chunk, need set loss-chunk size')
    group.add_argument('--eval-iteration', type=int, default=None,
                       help='eval iteration')
    group.add_argument('--data-cut-length', type=int, default=24000,
                       help='the length of data cut')
    return parser
...
def _add_moe_args(parser):
    ...
    group.add_argument('--use-attention-router', action='store_true',
                       help='use_attention_router.')
    group.add_argument('--selectable-expert', action='store_true',
                       help='selectable_expert.')
    group.add_argument('--per-layer-experts-blocks', type=str, default=None,
                       help='every layer expert num.')
...
def _add_experimental_args(parser):
    ...
    group.add_argument('--use-precision-aware-optimizer', action='store_true',
                       ...
                       'fp16 and fp8.')
    group.add_argument('--use-offload-precision-aware-optimizer', action='store_true',
                       help='Use the offload precision-aware optimizer in TransformerEngine, which allows '
                       'setting the main params and optimizer states to lower precision, such as '
                       'fp16 and fp8.')
