...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.compute_lm_head_fp32 = compute_lm_head_fp32
        ...
        if sequence_parallel:
            ...
            total_input = input
        if compute_lm_head_fp32:
            output = torch.matmul(total_input.to(torch.float32), weight.t().to(torch.float32))
        else:
    ...
    def backward(ctx, grad_output):
        ...
        compute_lm_head_fp32 = ctx.compute_lm_head_fp32
        wgrad_compute = True
        ...
        if wgrad_compute:
            if ctx.sequence_parallel:
                ...
                total_input = input
        if compute_lm_head_fp32:
            assert grad_output.dtype == torch.float32, f'check data dtype, grad_output.dtype={grad_output.dtype}'
            grad_input = grad_output.matmul(weight.to(torch.float32)).to(total_input)
        else:
            grad_input = grad_output.matmul(weight)
                ...
                if weight.main_grad.dtype == torch.float32:
                    if compute_lm_head_fp32:
                        ...
                        fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp32(
                            total_input.to(weight.main_grad.dtype), grad_output, weight.main_grad
                        )
                    else:
                        fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp32(
                    ...
                    if compute_lm_head_fp32:
                        ...
                        fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp16(
                            total_input, grad_output.to(weight.main_grad.dtype), weight.main_grad
                        )
                    else:
                        fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp16(
                            total_input, grad_output, weight.main_grad
...
def linear_with_grad_accumulation_and_async_allreduce(
    ...
    async_grad_allreduce: Optional[bool] = None,
    compute_lm_head_fp32: Optional[bool] = None
) -> torch.Tensor:
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def forward(
        ...
        compute_lm_head_fp32: Optional[bool] = False
    ):
        ...
        output_parallel = self._forward_impl(
            ...
            compute_lm_head_fp32=compute_lm_head_fp32
        )
