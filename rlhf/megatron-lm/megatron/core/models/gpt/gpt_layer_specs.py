...
from megatron.core.transformer.attention import SelfAttention, SelfAttentionSubmodules, LocalizedFiltering, LocalizedFilteringSubmodules
from megatron.core.transformer.dot_product_attention import DotProductAttention
from megatron.core.transformer.enums import AttnMaskType
from megatron.core.transformer.identity_op import IdentityOp
from megatron.core.transformer.mlp import MLP, MLPSubmodules, ImageMLP, ImageMLPSubmodules
...
def _get_lfa_module_spec(use_apex_rmsnorm) -> ModuleSpec:
    if use_apex_rmsnorm:
        output_layernorm = MixedFusedRMSNorm
    else:
    ...
    return ModuleSpec(
            ...
            submodules=LocalizedFilteringSubmodules(
                ...
                output_layernorm=output_layernorm #MixedFusedRMSNorm
                )
            )
...
def get_gpt_layer_with_transformer_engine_spec(
    ...
    """
    if use_lf_gate:
        lf_gate = _get_lfa_module_spec(use_apex_rmsnorm)
    ...
    if multi_latent_attention:
        ...
        elif use_apex_rmsnorm:
            pre_mlp_layernorm = MixedFusedRMSNorm
        else:
            ...
            submodules=TransformerLayerSubmodules(
                ...
                self_attention=ModuleSpec(
                    ...
                    submodules=SelfAttentionSubmodules(
                        linear_qkv=TELayerNormColumnParallelLinear if not use_lf_gate else IdentityOp,
                        linear_qk=TEColumnParallelLinear if use_lf_gate else IdentityOp,
                        linear_v=TEColumnParallelLinear if use_lf_gate else IdentityOp,
                        core_attention=TEDotProductAttention,
                        linear_proj=TERowParallelLinear,
                        q_layernorm=(
                        ...
                        lf_gate=lf_gate if use_lf_gate else IdentityOp,
                    ),
                ),
                self_attn_bda=get_bias_dropout_add,
...
def get_imagemlp_layer_with_transformer_engine_spec(
    use_apex_rmsnorm: bool = False,
    yuanvl_use_te_imagemlp: bool = False,
) -> ModuleSpec:

    if use_apex_rmsnorm:
        imagemlp_layernorm = MixedFusedRMSNorm
    else:
    ...
    return ModuleSpec(
        ...
        submodules=ImageMLPSubmodules(
            linear_fc1=TEColumnParallelLinear if yuanvl_use_te_imagemlp else ColumnParallelLinear,
            linear_fc2=TERowParallelLinear if yuanvl_use_te_imagemlp else RowParallelLinear,
            imagemlp_layernorm=imagemlp_layernorm,
        ),
    )
