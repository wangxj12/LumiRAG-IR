...
class TransformerConfig(ModelParallelConfig):
    ...
    selectable_expert: bool = False
    """ selectable expert """

    per_layer_experts_blocks: Optional[float] = None
    ...
    use_lf_gate: bool = False
    """When set to true, lfa will be used"""

    lf_conv2d_group: int = 1
    """When use_lf_gate set to true, set lf_conv2d_group"""

    lf_conv2d_num_pad: int = 1
    """When use_lf_gate set to true, set lf_conv2d_num_pad"""

    lf_conv2d_add_bias: bool = False
    """hen use_lf_gate set to true, set lf_conv2d_add_bias"""

    use_apex_rmsnorm: bool = False
    """When set to true, use apex Norm, not use TENorm"""
   
    compute_probs_in_model:  bool = False
    """ compute_probs_in_model, need use loss chunk"""
    
    compute_lm_head_fp32:  bool = False
    """ compute_lm_head_fp32, fp32 dtype will be used in lm head"""

    use_loss_chunk:  bool = False
    """"When set to true, loss_chunk will be used"""

    loss_chunk: int = 1024
    """loss_chunk size"""

    use_yuanvl: bool = False
    """When set to true, yuanvl will be used"""
    
    imagemlp_recompute: bool = False
    """When set to true, imagemlp_recompute will be used"""
    
    downsample_ratio: float = 0.5
    """The downsample ratio for iamge"""
    
    clip_hidden_size: int = 1024
    """The clip hidden size for Vit"""
    
    clip_visual_size: int = 1024
    """The clip visual size for Vit"""
    
    clip_model_name: str = "InternViT-448"
    """The clip model name for Vit"""
    
    clip_download_path: Optional[str] = None
    """The clip download path for Vit"""
    ...
    def __post_init__(self):
        ...
        if self.pipeline_model_parallel_method == 'block':
            assert self.pipeline_model_parallel_split_rank == None, 'pipeline_model_parallel_split_rank not support pipeline_model_parallel_method=block now'
            assert  self.encoder_pipeline_model_parallel_size == None or self.encoder_pipeline_model_parallel_size == 0, 'encoder_pipeline_model_parallel_size not support pipeline_model_parallel_method=block now'
            assert self.num_layers_per_virtual_pipeline_stage == None, 'pipeline interleavinig not support pipeline_model_parallel_method=block now'
            if isinstance(self.pipeline_model_parallel_blocks, str):
                self.pipeline_blocks = [int(i) for i in self.pipeline_model_parallel_blocks.split(',')]
            else:
                self.pipeline_blocks = self.pipeline_model_parallel_blocks
            assert sum(self.pipeline_blocks) == self.num_layers, 'sum of args.pipeline_model_parallel_blocks must eq args.num_layers'
            assert len(self.pipeline_blocks) == self.pipeline_model_parallel_size, 'num of args.pipeline_model_parallel_blocks must eq args.pipeline_model_parallel_size'
            if self.standalone_embedding_stage:
                print('standalone_embedding_stage can not be used with pipeline_model_parallel_method=block')
                exit()

        else:
            assert self.num_layers % self.pipeline_model_parallel_size == 0
            self.pipeline_blocks = [self.num_layers // self.pipeline_model_parallel_size] * self.pipeline_model_parallel_size
        ...
        if (
            ...
            or self.num_layers_in_last_pipeline_stage is not None
                ...
                self.pipeline_model_parallel_method == 'block'
        ):
            raise ValueError(
                'num_layers_in_first_pipeline_stage and num_layers_in_last_pipeline_stage cannot be'
                'set at the same time with pipeline_model_parallel_method'
            )        
        ...
        if (
            ...
            raise ValueError(
                'self.account_for_loss_in_pipeline_split and self.account_for_embedding_in_pipeline_split cannot be'
                'set at the same time with pipeline_model_parallel_method'
            )        
