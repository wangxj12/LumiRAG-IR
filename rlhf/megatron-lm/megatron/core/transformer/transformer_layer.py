        ...
        if (
            ...
            if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
                ...
                if parallel_state.get_pipeline_model_parallel_world_size() > 1:
                    ...
                    if pipeline_this_ranks == 0:
                        offset = 0
                    else:
                        offset = sum(pipeline_blocks[:pipeline_this_ranks])
                else:
                    offset = 0
                # Reduce the offset of embedding layer from the total layer number
                if (
...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def __init__(
        ...
        # [Module 8: MLP block]
        self.mlp = build_module(submodules.mlp, config=self.config, layer_number=self.layer_number)
