...
def get_num_layers_to_build(config: TransformerConfig) -> int:
    ...
    if (
        ...
        if config.pipeline_model_parallel_method != 'block':
            assert (
    ...
    if (
        ...
        pipeline_blocks = config.pipeline_blocks
        pipeline_this_ranks = parallel_state.get_pipeline_model_parallel_rank()
        num_layers_to_build = pipeline_blocks[pipeline_this_ranks]
...
class NoopTransformerLayer(MegatronModule):
    ...
    def forward(self,
        ...
        if context == None:
            return hidden_states.clone(), None
        else:
            return hidden_states.clone(), context.clone()
...
class TransformerBlock(MegatronModule):
    ...
    def _build_layers(self):
        ...
        if len(self.submodules.layer_specs) != 0:
            ...
            self.layers = torch.nn.ModuleList([NoopTransformerLayer(self.config, 1)])
    ...
    def _checkpointed_forward(
        ...
        def custom(start: int, end: int):
            def custom_forward(
                hidden_states, attention_mask, context, context_mask, rotary_pos_emb, position_ids
            ):
