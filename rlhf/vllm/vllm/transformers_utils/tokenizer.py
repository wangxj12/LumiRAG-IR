...
setattr(SpecialTokensMixin, "all_special_tokens_yuanvl", None)
setattr(SpecialTokensMixin, "all_special_ids_yuanvl", None)

class YuanSpecialTokensMixin(SpecialTokensMixin):
    @property
    def all_special_tokens(self) -> List[str]:
        if self.all_special_tokens_yuanvl is not None:
            return self.all_special_tokens_yuanvl

        all_toks = [str(s) for s in self.all_special_tokens_extended]
        self.all_special_tokens_yuanvl = list(set(all_toks))
        return all_toks

    @property
    def all_special_ids(self) -> List[int]:
        if self.all_special_ids_yuanvl is not None:
            return self.all_special_ids_yuanvl

        all_toks = self.all_special_tokens
        all_ids = self.convert_tokens_to_ids(all_toks)
        self.all_special_ids_yuanvl = list(set(all_ids))

        return all_ids

SpecialTokensMixin.all_special_tokens = YuanSpecialTokensMixin.all_special_tokens
SpecialTokensMixin.all_special_ids = YuanSpecialTokensMixin.all_special_ids
