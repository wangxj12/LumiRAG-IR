...
class ModelInputForGPUBuilder(ModelRunnerInputBuilderBase[ModelInputForGPU]):
    ...
    def _compute_lens(self, inter_data: InterDataForSeqGroup, seq_idx: int,
        ...
        mm_offset = seq_data.get_mm_offset()
        ...
        if mm_offset > 0:
            if inter_data.is_prompt:
                if context_len > mm_offset:
                    # prefix cache
                    prompt_tokens_len = seq_len + context_len - mm_offset
                    positions = list(range(context_len - mm_offset, prompt_tokens_len))
                elif seq_len + context_len < mm_offset:
                    # chunked cache
                    positions = [0] * seq_len
                else:
                    prompt_tokens_len = seq_len + context_len - mm_offset
                    positions = [0] * (mm_offset - context_len) + list(range(prompt_tokens_len))
            else:
                # decode
                positions = [context_len - mm_offset]
            inter_data.input_positions[seq_idx].extend(positions)
        else:
            inter_data.input_positions[seq_idx].extend(range(context_len, seq_len))
