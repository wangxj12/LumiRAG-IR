...
def init_config(n_gpus_per_node, pp_size=1) -> DictConfig:
    ...
    config.actor_rollout_ref.model.path = "/workspace/Qwen2-7B-Instruct"
...
def initialize(config, backend) -> tuple[AgentLoopManager | RayWorkerGroup, StatefulDataLoader]:
    ...
    dataset = RLHFDataset(
        data_files=os.path.expanduser("/workspace/gsm8k/train.parquet"),
        tokenizer=tokenizer,
        config=config.data,
    )
...
def perf_rollout(mode, backend, pp_size, n_gpus_per_node, num_steps):
    ...
    for step, batch in enumerate(dataloader):
        ...
        t_end = time.time()
        print(
            f"[DEBUG] backend: {backend}, pp_size: {pp_size}, n_gpus_per_node: {n_gpus_per_node}, batch_size: {len(gen_batch)}, "
