...
from megatron.data.yuanvl_dataset import dynamic_preprocess as mg_dynamic_preprocess

def process_image(image: dict | Image.Image, image_processor=None, image_segment_method='dynamic', input_size=448, max_num=9) -> Image.Image:
    ...
    if "bytes" in image:
        ...
        if image_processor == None:
            ...
            if image_segment_method == 'dynamic':
                images_processed = mg_dynamic_preprocess(image, image_size=input_size, use_thumbnail=True, max_num=max_num)
            pixel_values = [image_processor(images=image, return_tensors='pt').pixel_values.squeeze(0) for image in images_processed]
            return pixel_values
    
    if isinstance(image, str) and image.startswith('/'): # 若输入为图片路径时会用到
        image_obj = Image.open(image)
        image = image_obj.convert("RGB")
        if image_processor == None:
            return image
        else:
            if image_segment_method == 'adaptive':
                images_processed = mg_process_image(image, input_size, max_num)
            elif image_segment_method == 'dynamic':
                images_processed = mg_dynamic_preprocess(image, image_size=input_size, use_thumbnail=True, max_num=max_num)
            pixel_values = [image_processor(images=image, return_tensors='pt').pixel_values.squeeze(0) for image in images_processed]
            return pixel_values
