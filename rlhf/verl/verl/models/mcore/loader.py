...
def load_state_dict_to_megatron_gptmodel(state_dict, wrapped_models, config, params_dtype, is_value_model=False):
    ...
    if dp_rank == 0:
        ...
        for layer in range(config.num_hidden_layers):
            ...
            # if all(name in ["q_proj", "k_proj", "v_proj"] for f"{layer_name}.self_attn.{name}.weight" in state_dict):
            ...
            if hasattr(sync_layer.mlp, 'linear_fc1'):
                _broadcast_tensor(
            ...
            if all(f"{layer_name}.mlp.experts.{name}.0.weight" in state_dict for name in ['w1', 'w2']):
                num_experts_per_partition = sync_layer.mlp.config.num_moe_experts // mpu.get_expert_model_parallel_world_size()
                ep_rank = mpu.get_expert_model_parallel_rank()
                ep_offset = ep_rank * num_experts_per_partition
                for exp_id in range(sync_layer.mlp.num_local_experts):
                    _broadcast_tp_shard_tensor_w1(
                        getattr(sync_layer.mlp.experts.linear_fc1, f'weight{exp_id}') if dst_pp_rank == pp_rank else None,
                        f"{layer_name}.mlp.experts.w1.{exp_id + ep_offset}.weight",
                    )
                    _broadcast_tp_shard_tensor(
                        getattr(sync_layer.mlp.experts.linear_fc2, f'weight{exp_id}') if dst_pp_rank == pp_rank else None,
                        f"{layer_name}.mlp.experts.w2.{exp_id + ep_offset}.weight",
                        chunk_dim=1,
                    )
                print_rank_0(f"{layer_name}.mlp.experts.w1 weights and {layer_name}.mlp.experts.w2 weights finished")
