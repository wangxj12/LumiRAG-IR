...
def compute_policy_loss_sapo(
    ...
    """
    Compute the clipped policy objective and related metrics for SAPO.
    ...
    Args:
        ...
        response_mask (torch.Tensor):
            Mask indicating which tokens to include in the loss, shape (batch_size, response_length).
        loss_agg_mode (str, optional):
            Aggregation mode for `agg_loss`. For SAPO, it is recommended to use "token-mean".
    """

    assert config is not None
    assert isinstance(config, ActorConfig)
    tau_neg = config.clip_ratio_low if config.clip_ratio_low is not None else config.clip_ratio
    tau_pos = config.clip_ratio_high if config.clip_ratio_high is not None else config.clip_ratio
    ...
    def soft_clipped_ratio(tau):
        return torch.sigmoid( tau * (ratio - 1) ) * ( 4 / tau )

    soft_clipped_ratio = torch.where(advantages > 0, soft_clipped_ratio(tau_pos), soft_clipped_ratio(tau_neg))
    pg_losses = -advantages * soft_clipped_ratio
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
...
@register_policy_loss("eighty-twenty")
def compute_policy_loss_eighty_twenty(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    ...
    """
    Compute the clipped policy objective and related metrics for case15-1.
    ...
    Args:
        ...
        advantages (torch.Tensor):
            Advantage estimates for each action, shape (batch_size, response_length).
        entropy (toch.Tensor):
            entropy calculate for each generation, shape (batch_size, response_length).
        response_mask (torch.Tensor):
            Mask indicating which tokens to include in the loss, shape (batch_size, response_length).
        loss_agg_mode (str, optional):
            Aggregation mode for `agg_loss`. Defaults to "token-mean".
    ...
    pg_losses = torch.where(advantages < 0, pg_losses2, clip_pg_losses1)

    # find entropy threshold, ascend by default, 0.8-> 0.2 of descend
    entropy_threshold = torch.quantile(entropy[response_mask==1.0], 0.8)
    assert entropy.shape == pg_losses.shape
    pg_losses = torch.where(entropy >= entropy_threshold, pg_losses, pg_losses * 0)
    print(f"eighty-twenty overhere!!!")
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

    # 0.0 for pg_clipfrac_lower
    return pg_loss, pg_clipfrac, ppo_kl, torch.tensor(0.0)



@register_policy_loss("eighty-twenty-overlong-filter")
def compute_policy_loss_eighty_twenty(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    ...
    if overlong_filter_flag:
        pg_loss *= 0
        print(f"overlong-filer overhere!!!")
