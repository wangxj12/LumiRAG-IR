...
class MegatronWorker(Worker):
    ...
    def _init_hf_config_and_tf_config(
        ...
        # Step 2: get the hf
        if any(name in self.local_path for name in ['yuanvl', 'YuanVL']):
            from verl.models.configs.configuration_yuanvl import YuanVLChatConfig
            hf_config = YuanVLChatConfig.from_pretrained(self.local_path, trust_remote_code=trust_remote_code)
        elif any(name in self.local_path for name in ['yuan', 'Yuan']):
            from verl.models.configs.configuration_yuan import YuanConfig
            hf_config = YuanConfig.from_pretrained(self.local_path, trust_remote_code=trust_remote_code)
        else:
        ...
        if hasattr(hf_config, 'llm_config'):
            self.share_embeddings_and_output_weights = getattr(hf_config.llm_config, "tie_word_embeddings", False)
        else:
