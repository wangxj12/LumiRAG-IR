...
def all_gather_data_proto(data: DataProto, process_group):
    ...
    has_images = False
    if 'processed_images' in data.non_tensor_batch:
        has_images = True
        images_tensor_batch = data.non_tensor_batch.pop('processed_images')
        nlength = len(images_tensor_batch)
        if nlength <= 2:
            split_n = nlength
        else:
            split_n = 3
            while split_n <= nlength and nlength % split_n != 0:
                split_n += 1

        images_tensor_batch_parts = np.array_split(images_tensor_batch, split_n)
        
        img_non_tensor_batchs = [None for _ in range(group_size*split_n)]
        for i, batch in enumerate(images_tensor_batch_parts):
            img_non_tensor_batch = [None for _ in range(group_size)]
            torch.distributed.all_gather_object(img_non_tensor_batch, batch, group=process_group)
            for j in range(group_size):
                img_non_tensor_batchs[i+j*split_n] = img_non_tensor_batch[j]
    ...
    if has_images:
        data.non_tensor_batch['processed_images'] = np.concatenate(img_non_tensor_batchs)
