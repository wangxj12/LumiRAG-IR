...
class MegatronPPOActor(BasePPOActor):
    ...
    def make_minibatch_iterator(self, data: DataProto) -> Iterable[DataProto]:
        ...
        if self.has_multi_modal_inputs:
            data = data.select(select_keys, ["multi_modal_inputs", 'processed_images', 'extra_info'])
        else:
    ...
    def forward_backward_batch(
        ...
        if use_dynamic_bsz:
            ...
            if 'processed_images' in mini_batch.non_tensor_batch:
                assert micro_batch_size == 1, f'yuanvl 多模态任务只支持'
                processed_images = []
                num_data = 0
                for images in mini_batch.non_tensor_batch['processed_images']:
                    try:
                        if (isinstance(images, np.ndarray) or isinstance(images, list)) and (len(images) > 0): 
                            processed_images.append(torch.tensor(images[0].astype(np.float32)) if isinstance(images[0][0], np.ndarray) else images[0])
                        else:
                        ...
                        exit()
                extra_info_list = []
                for extra_info_in in mini_batch.non_tensor_batch['extra_info']:
                    extra_info_list.append(extra_info_in)

                for i, batch in enumerate(micro_batches):
                    batch['processed_images'] = processed_images[i*micro_batch_size:(i+1)*micro_batch_size]
                    batch['extra_info_list'] = extra_info_list[i*micro_batch_size:(i+1)*micro_batch_size]
        ...
        def loss_func(output, data, meta_info):
            ...
            if not forward_only:
                ...
                elif loss_mode == "eighty-twenty-overlong-filter":
                    extra_info_list = data['extra_info_list'].tolist()
                    # the following works only for micro_batch_size = 1
                    overlong_filter_flag = extra_info_list[0]['overlong_filter_flag']
                    entropy = output["entropy"][:, -response_length - 1 : -1].contiguous()
                    pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = policy_loss_fn(
                        old_log_prob=old_log_prob,
                        log_prob=log_prob,
                        ...
                        response_mask=response_mask,
                        loss_agg_mode=loss_agg_mode,
                        overlong_filter_flag=overlong_filter_flag,
                        config=self.config,
                    )
        ...
        def get_labelmask_for_yuanvl(label_mask):
            # 找到最后一个True的位置
            ones = (label_mask[0] == True).nonzero(as_tuple=True)[0]
            if len(ones) > 0:
                last_idx = ones[-1]
                label_mask[0, last_idx] = False
            return label_mask
        ...
        def forward_step(batch_iter, model):
            ...
            if 'processed_images' in batch:
                processed_images = batch['processed_images']
            else:
                processed_images = None

            if 'extra_info_list' in batch:
                extra_info_list = batch['extra_info_list']
            else:
            ...
            #对于 YuanVL label_mask, 修正因为eod带来的问题, 这里的代码仅支持mbs=1
            assert len(attention_mask) == 1, '对于 YuanVL label_mask, 修正因为eod带来的问题, 这里的代码仅支持mbs=1'
            if attention_mask[:, -1] == False:
                label_mask = get_labelmask_for_yuanvl(label_mask)
            label_mask[:, -1] = False
    ...
    def update_policy(self, dataloader: Iterable[DataProto]) -> dict:
        ...
        for data in dataloader:
            ...
            calculate_entropy = (self.config.entropy_coeff != 0 or "eighty-twenty" in self.config.policy_loss.get("loss_mode", "vanilla"))
